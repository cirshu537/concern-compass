-- Create function to handle review-based credit awards and negative events
...
BEGIN
  ...
  -- Handle cancelled complaints - automatic negative for student
  IF v_complaint.status = 'cancelled' AND NEW.reviewer_role IN ('staff', 'branch_admin') THEN
    -- Record negative event for student
    INSERT INTO negative_events (user_id, complaint_id)
    VALUES (v_complaint.student_id, NEW.complaint_id)
    ...
    -- Check if student should be banned
    SELECT * INTO v_student_profile FROM profiles WHERE id = v_complaint.student_id;
    IF v_student_profile.negative_count_lifetime >= 3 THEN
      UPDATE profiles 
      SET banned_from_raise = true 
      WHERE id = v_complaint.student_id;
    END IF;
...
DROP TRIGGER IF EXISTS on_review_submitted ON complaint_reviews;
